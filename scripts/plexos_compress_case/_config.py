...
"""Configuration for plexos_compress_case.
...
(codec, codec_args, unwrap) live in ``[plexos_compress_case]``.
...
codec_args
    Extra arguments for the codec command.  Default ``"-T0 -9"``
    (multi-threaded xz at max level — the inner zips already use
    deflate, so the highest level is needed to extract a meaningful
    second-pass gain).
unwrap
    Whether to unwrap ``PLEXOS{YYYYMMDD}.zip`` outer bundles into their
    inner ``DATOS{date}.zip`` / ``RES{date}.zip`` payloads on compress
    and recombine them on decompress.  The outer wrapper just
    ``store``-bundles the two inner zips and adds nothing recoverable;
    dropping it shrinks repo storage by ~30 MB per bundle.  Default
    ``"true"``.
"""
...
def load_config(config_path: Path | None = None) -> dict[str, str]:
    """Load plexos_compress_case settings from the unified config.

    Reads color from ``[global]`` and tool-specific settings from
    ``[plexos_compress_case]``.  Returns a flat dict.
    """
