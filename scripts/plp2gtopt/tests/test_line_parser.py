...
def test_parse_file(line_parser):
    """Test parsing the line data file."""
    assert line_parser.get_num_lines() == 10
    lines = line_parser.get_lines()
    assert len(lines) == 10


def test_line_data(line_parser):
    """Test line data structure."""
    line = line_parser.get_line_by_name("Andes220->Oeste220")
...
def test_get_lines_by_bus(line_parser):
    """Test getting lines by bus name."""
    lines = line_parser.get_lines_by_bus("Andes220")
    assert len(lines) == 2
...
def test_get_lines_by_bus_num(line_parser):
    """Test getting lines by bus number."""
    lines = line_parser.get_lines_by_bus_num(5)
    assert len(lines) == 3
    names = {line["name"] for line in lines}
...
def test_voltage_parsing(line_parser):
    """Test voltage extraction from line names."""
    line = line_parser.get_line_by_name("Capricorn220->Capricorn110")
    assert line["voltage"] == 220.0
    line = line_parser.get_line_by_name("Antofag110->Desalant110")
    assert line["voltage"] == 110.0


def test_operational_status(line_parser):
