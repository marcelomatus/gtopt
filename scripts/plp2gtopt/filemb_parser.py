...
"""Parser for plpfilemb.dat format files containing reservoir filtration data.

This is the primary filtration model used by PLP (analogous to plpcenre.dat
for reservoir efficiencies).

Handles:
- File parsing and validation
- Filtration data structure creation (piecewise-linear segments)
...
Field definitions:
  ...
  mean_filtration – Mean/fallback filtration flow [m³/s]
  ...
  constant        – Filtration rate at the volume breakpoint [m³/s] (no
                    unit conversion needed).
  CENTRAL_NAME    – Receiving central name (destination of filtration water)
...
Equivalent gtopt JSON fields:
  Filtration.slope     (scalar or schedule) ← segment slope for SDDP update
  Filtration.constant  (scalar or schedule) ← segment constant for SDDP update
  Filtration.segments  ← list of {volume, slope, constant} dicts
...
class FilembParser(BaseParser):
    """Parser for plpfilemb.dat files containing reservoir filtration data.

    This parser is the primary source for filtration data in PLP cases
    and is analogous to CenreParser for reservoir efficiencies.

    The piecewise-linear segments it provides are used by FiltrationLP to
    ...
    def parse(self, parsers: Optional[Dict[str, Any]] = None) -> None:
        ...
        Reads piecewise-linear filtration curves from the PLP file format.
        ...
        for _ in range(num_entries):
            ...
            if idx >= len(lines):
                raise ValueError(
                    "Unexpected end of plpfilemb.dat file (mean filtration)."
                )

            # Mean/average filtration flow [m³/s]
            mean_filtration = max(self._parse_float(lines[idx]), 0.0)
            idx += 1
            ...
            # Receiving central name (destination of filtration water)
            ...
            entry: Dict[str, Any] = {
                ...
                "mean_filtration": mean_filtration,
                "segments": segments,
            }
            self._append(entry)
    ...
    def get_filtration_by_embalse(self, embalse_name: str) -> Optional[Dict[str, Any]]:
        """Get filtration data by embalse (source reservoir) name."""
