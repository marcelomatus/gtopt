...
LineLP::LineLP(const InputContext& ic, Line&& pline)
    ...
    , bus_a_index(ic.element_index(bus_a()))
    , bus_b_index(ic.element_index(bus_b()))
{
}

bool LineLP::add_to_lp(const SystemContext& sc, LinearProblem& lp)
{
  ...
  auto&& bus_a = sc.element(bus_a_index);
  auto&& bus_b = sc.element(bus_b_index);
  if (!bus_a.is_active(stage_index) || !bus_b.is_active(stage_index)) {
    return true;
  }
  ...
  const auto stage_lossfactor = sc.stage_lossfactor(lossfactor);
       ...
       std::ranges::views::zip(block_indexes, balance_rows_a, balance_rows_b))
  {
    ...
    const auto block_tcost = sc.block_cost(block, stage_tcost);
    ...
    if (!has_loss || block_tmax > 0.0) {
      ...
      const auto fpc =
          lp.add_col({// flow variable
                      .name = sc.stb_label(block, cname, "fp", uid()),
      ...
      if (capacity_col) {
        SparseRow cprow {.name = sc.stb_label(block, cname, "capp", uid())};
    ...
    if (has_loss && block_tmin < 0.0) {
      ...
      const auto fnc =
          lp.add_col({// flow variable
                      .name = sc.stb_label(block, cname, "fn", uid()),
                      .lowb = 0,
                      .uppb = -block_tmin,
                      .cost = block_tcost});
      ...
      // adding the capacity constraint
      if (capacity_col) {
        SparseRow cnrow {.name = sc.stb_label(block, cname, "capn", uid())};
  ...
  if (const auto& stage_reactance = sc.stage_reactance(reactance)) {
    const auto& theta_a_cols = bus_a.theta_cols_at(sc, lp, blocks);
    const auto& theta_b_cols = bus_b.theta_cols_at(sc, lp, blocks);
    ...
    if (!theta_a_cols.empty() && !theta_b_cols.empty()) {
      ...
      for (const auto block_index : block_indexes) {
        SparseRow trow(
            {.name = sc.stb_label(blocks[block_index], cname, "theta", uid())});
        trow.reserve(4);
      ...
      if (!sc.emplace_bholder(theta_rows, std::move(trows)).second) {
        return false;
      }
    }
  }
  ...
  return sc.emplace_bholder(capacityp_rows, std::move(cprows)).second
      && sc.emplace_bholder(capacityn_rows, std::move(cnrows)).second
      && sc.emplace_bholder(flowp_cols, std::move(fpcols)).second
      && sc.emplace_bholder(flown_cols, std::move(fncols)).second;
}
