...
struct ArrayIndexBase
{
      ...
      auto& table_map)
  {
    ...
    SPDLOG_DEBUG("get_arrow_index: looking for column {}", name);
    ...
    auto array = [&]()
    {
      ...
      if (auto col = table->GetColumnByName(name)) {
        SPDLOG_DEBUG("Found column '{}' for uid '{}'", name, uid);
        return col;
      }
      ...
      // Try original-case name:uid (as_label lowercases; Parquet files
      // generated by external tools may keep the original case).
      const auto orig_case = std::format("{}:{}", name, uid);
      if (auto col = table->GetColumnByName(orig_case)) {
        SPDLOG_DEBUG("Found column '{}' for uid '{}'", orig_case, uid);
        return col;
      }
      // Try bare uid as string
      const auto uid_str = std::to_string(uid);
      if (auto col = table->GetColumnByName(uid_str)) {
        SPDLOG_DEBUG("Found column '{}' for uid '{}'", uid_str, uid);
        return col;
      }
    ...
    if (!array) [[unlikely]] {
      // Collect existing column names for a helpful error message
      std::string available;
      for (int ci = 0; ci < table->num_columns(); ++ci) {
        if (!available.empty()) {
          available += ", ";
        }
        available += table->field(ci)->name();
      }
      const auto msg = std::format(
          "Can't find element '{}:{}' in table '{}'. "
          "Available columns: [{}]",
          name,
          uid,
          fname,
