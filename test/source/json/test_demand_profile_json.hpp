...
TEST_CASE("DemandProfile daw json test - with cost")
{
  ...
  CHECK(std::get<IntBool>(dp.active.value()) == True);  // NOLINT(bugprone-unchecked-optional-access)
  CHECK(std::get<Name>(dp.demand) == "DEMAND_REF");
  REQUIRE(dp.scost.has_value());
  CHECK(std::get<double>(dp.scost.value())  // NOLINT(bugprone-unchecked-optional-access)
