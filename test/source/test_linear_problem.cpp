...
TEST_CASE("Linear problem test 2")
{
  ...
  {
    const gtopt::SparseCol col({.lowb = 25, .uppb = 25});
  ...
  {
    const SparseCol col({.lowb = -25, .uppb = 25, .cost = 10});
...
TEST_CASE("Linear problem matrix operations")
{
  ...
  for (int i = 0; i < 5; ++i) {
    col_indices.push_back(lp.add_col(gtopt::SparseCol {}));
  }

  std::vector<RowIndex> row_indices;
  row_indices.reserve(3);
  for (int i = 0; i < 3; ++i) {
    row_indices.push_back(lp.add_row(gtopt::SparseRow {}));
  }
...
TEST_CASE("Linear problem edge cases")
{
  ...
  {
    auto row_idx = lp.add_row(gtopt::SparseRow {});
    auto col_idx = lp.add_col(gtopt::SparseCol {});
...
TEST_CASE("Linear problem advanced operations")
{
  ...
  const auto col1 = lp.add_col(SparseCol {});
  const auto col2 = lp.add_col(SparseCol {}.free());
  ...
  const auto row1 = lp.add_row(gtopt::SparseRow {});
  const auto row2 = lp.add_row(gtopt::SparseRow {});
...
TEST_CASE("Linear problem flatten row ordering")
{
  ...
  const auto c0 = lp.add_col(gtopt::SparseCol {});
  const auto c1 = lp.add_col(gtopt::SparseCol {});

  auto r0 = lp.add_row(gtopt::SparseRow {});
  auto r1 = lp.add_row(gtopt::SparseRow {});
  auto r2 = lp.add_row(gtopt::SparseRow {});
...
TEST_CASE("Linear problem flatten column and row names")
{
  ...
  // Columns and rows have no .name field; names are generated from metadata.
  // Without metadata, all names are empty strings.
  const auto c0 = lp.add_col(gtopt::SparseCol {});
  const auto c1 = lp.add_col(gtopt::SparseCol {});
  const auto c2 = lp.add_col(gtopt::SparseCol {});
  ...
  SUBCASE("row_with_names only (no maps)")
  {
    ...
    CHECK(flat.rownm[0].empty());
    CHECK(flat.rownm[1].empty());
  ...
  SUBCASE("col_with_name_map with empty names produces empty map")
  {
    ...
    // Empty names are skipped in the name map
    CHECK(flat.colmp.empty());
  ...
  SUBCASE("row_with_name_map with empty names produces empty map")
  {
    const auto flat = lp.flatten({
        .col_with_names = false,
        .col_with_name_map = false,
  ...
  SUBCASE("all names and maps enabled with empty names")
  {
    const auto flat = lp.flatten({
        .col_with_names = true,
        .row_with_names = true,
  ...
  SUBCASE("col_with_name_map with empty names builds empty map")
  {
    const auto flat = lp.flatten({
        .col_with_name_map = true,
    });
    ...
    "Linear problem flatten compute_stats with col names and zeroed count")  // NOLINT
{
  ...
  auto r1 = lp.add_row(SparseRow {});
  auto r2 = lp.add_row(SparseRow {});
...
TEST_CASE("flatten name map skips empty column names")
{
  ...
  [[maybe_unused]] const auto c0 = lp.add_col(gtopt::SparseCol {});
  [[maybe_unused]] const auto c1 = lp.add_col(gtopt::SparseCol {});
  [[maybe_unused]] const auto c2 = lp.add_col(gtopt::SparseCol {});
  [[maybe_unused]] const auto c3 = lp.add_col(gtopt::SparseCol {});

  // flatten requires at least one row to produce output
  auto rrow = gtopt::SparseRow {};
  rrow[c0] = 1;
  ...
  // All 4 column names should be present in colnm (all empty, no .name field)
  ...
  // The name map should be empty (all names are empty)
  CHECK(flat.colmp.empty());
}
...
TEST_CASE("flatten per-row-type coefficient stats")  // NOLINT
{
  ...
  // Verify that row_type_stats is empty when rows have no metadata.
  LinearProblem lp("row_type_stats_test");

  const auto c0 = lp.add_col(SparseCol {.cost = 1.0});
  const auto c1 = lp.add_col(SparseCol {.cost = 2.0});

  auto r_bal0 = SparseRow {};
  r_bal0[c0] = 1.0;
  r_bal0[c1] = 100.0;
  r_bal0.equal(50.0);
  std::ignore = lp.add_row(std::move(r_bal0));

  auto r_bal1 = SparseRow {};
  r_bal1[c0] = 0.5;
  r_bal1[c1] = 200.0;
  r_bal1.equal(100.0);
  std::ignore = lp.add_row(std::move(r_bal1));

  auto r_theta = SparseRow {};
  r_theta[c0] = 0.001;
  r_theta[c1] = 1000.0;
  r_theta.equal(0.0);
  ...
  // Without metadata, row names are empty → all rows bucketed as "unknown"
  REQUIRE(flat.row_type_stats.size() == 1);
  CHECK(flat.row_type_stats[0].type == "unknown");
  CHECK(flat.row_type_stats[0].count == 3);
}
...
{  // NOLINT
  ...
  const auto c0 = lp.add_col(SparseCol {.cost = 1.0});
  const auto c1 = lp.add_col(SparseCol {.cost = 1.0});
