 ...
 * columns created by the earlier elements).  `FutureCostLP` follows it but is
 * planning-only (no per-(scenario, stage) `add_to_lp`), so it does not affect
 * the operational build order; it is placed after both ReservoirLP and
 * UserConstraintLP so its α cut can reference their terminal state columns.
 */
