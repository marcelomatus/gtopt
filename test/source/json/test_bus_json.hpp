...
TEST_CASE("Bus with active property serialization")
{
  SUBCASE("With boolean active")
  {
    ...
    CHECK(std::get<IntBool>(roundtrip.active.value()) == True);  // NOLINT(bugprone-unchecked-optional-access)
  }
  ...
  SUBCASE("With schedule active")
  {
    ...
    REQUIRE(roundtrip.active.has_value());
    const auto& active =
        std::get<std::vector<IntBool>>(roundtrip.active.value());  // NOLINT(bugprone-unchecked-optional-access)
