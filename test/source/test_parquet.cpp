...
TEST_CASE("Parquet read int16 columns as int32")
{
  ...
  SUBCASE("Write parquet with int16 columns and read back")
  {
    ...
    CHECK(result->at(
          ...
          == 0);
    CHECK(result->at(
              {make_uid<Scenario>(1), make_uid<Stage>(2), make_uid<Block>(1)})
          == 1);
    CHECK(result->at(
              {make_uid<Scenario>(2), make_uid<Stage>(1), make_uid<Block>(1)})
          == 2);
    CHECK(result->at(
              {make_uid<Scenario>(2), make_uid<Stage>(2), make_uid<Block>(1)})
          == 3);
  }
}
...
TEST_CASE("Parquet file write and read test 2")
{
  ...
  SUBCASE("Read and verify Parquet file")
  {
    ...
    CHECK(scenario_array->Value(1) == 1);
    CHECK(scenario_array->Value(2) == 1);
    CHECK(scenario_array->Value(3) == 1);
    CHECK(scenario_array->Value(4) == 1);

    // Verify 'stage' column data
    auto stage_array =
        std::static_pointer_cast<arrow::Int32Array>(stage_column->chunk(0));
    CHECK(stage_array->Value(0) == 1);
    CHECK(stage_array->Value(1) == 2);
    CHECK(stage_array->Value(2) == 3);
    CHECK(stage_array->Value(3) == 4);
    CHECK(stage_array->Value(4) == 5);

    // Verify 'block' column data
    auto block_array =
        std::static_pointer_cast<arrow::Int32Array>(block_column->chunk(0));
    CHECK(block_array->Value(0) == 1);
    CHECK(block_array->Value(1) == 2);
    CHECK(block_array->Value(2) == 3);
    CHECK(block_array->Value(3) == 4);
    CHECK(block_array->Value(4) == 5);

    // Verify 'd1' column data
    auto d1_array =
        std::static_pointer_cast<arrow::DoubleArray>(d1_column->chunk(0));
    CHECK(d1_array->Value(0) == doctest::Approx(1.0));
    CHECK(d1_array->Value(1) == doctest::Approx(2.0));
    CHECK(d1_array->Value(2) == doctest::Approx(3.0));
    CHECK(d1_array->Value(3) == doctest::Approx(4.0));
    CHECK(d1_array->Value(4) == doctest::Approx(5.0));
  ...
  SUBCASE("schedule parquet test")
  {
    ...
    SUBCASE("stbfield")
    {
      ...
      REQUIRE(stbsched.at(
              ...
              == 2);
      REQUIRE(stbsched.at(
                  make_uid<Scenario>(1), make_uid<Stage>(3), make_uid<Block>(3))
              == 3);
      REQUIRE(stbsched.at(
                  make_uid<Scenario>(1), make_uid<Stage>(4), make_uid<Block>(4))
              == 4);
      REQUIRE(stbsched.at(
                  make_uid<Scenario>(1), make_uid<Stage>(5), make_uid<Block>(5))
              == 5);
    }

    SUBCASE("stfield")
    {
      const TBRealFieldSched tbfield {cname + "@" + fname};
      ...
      REQUIRE(tbsched.at(make_uid<Stage>(3), make_uid<Block>(3)) == 3);
      REQUIRE(tbsched.at(make_uid<Stage>(4), make_uid<Block>(4)) == 4);
      REQUIRE(tbsched.at(make_uid<Stage>(5), make_uid<Block>(5)) == 5);
    }
