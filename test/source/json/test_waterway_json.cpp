...
TEST_CASE("Waterway JSON basic parsing")
{
  ...
  REQUIRE(std::get<gtopt::Uid>(waterway.junction_b.value()) == 20);
...
TEST_CASE("Waterway JSON with active schedule")
{
  ...
  const Active active_val =
      waterway.active.value_or(Active {std::vector<IntBool> {}});
...
TEST_CASE("Waterway JSON roundtrip serialization")
{
  ...
  REQUIRE(roundtrip.junction_b.has_value());
  REQUIRE(std::get<gtopt::Uid>(roundtrip.junction_b.value()) == 2);
