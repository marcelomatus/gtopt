#!/usr/bin/env python3
"""
make_igtopt_template.py — Generate the gtopt Excel template from C++ source.

This tool reads the gtopt C++ JSON header files (``include/gtopt/json/``) to
discover which arrays and fields belong to the JSON schema, then generates an
Excel workbook suitable for use with the ``igtopt`` converter.

Usage
-----
    python scripts/make_igtopt_template.py [--header-dir DIR] [--output FILE]

If ``--header-dir`` is omitted the script searches ``include/gtopt/json/``
relative to the repository root (the directory that contains the ``include/``
sub-tree).

The generated workbook contains:

- A ``.introduction`` sheet with an overview of the gtopt planning format
  and a directory of all sheets.
- An ``options`` sheet with every supported option key (one key per row) and
  the corresponding description / default value in adjacent columns.
- All simulation sheets (``block_array``, ``stage_array``, ``scenario_array``,
  ``phase_array``, ``scene_array``).
- All system sheets (``bus_array``, ``generator_array``, etc.) derived
  directly from ``json_system.hpp``.

Automatic sync with C++
-----------------------
Re-running this script after adding a new JSON element to the C++ headers
produces an updated template that already contains the new sheet and its
fields.  The ``igtopt`` expected-sheet list (``_SYSTEM_SHEETS`` / ``_SIMULATION_SHEETS``)
must be updated separately — that list is derived from the JSON header files
and can be inspected with ``--list-sheets``.

Examples
--------
    # Generate template in docs/templates/ (default output location)
    python scripts/make_igtopt_template.py

    # Write to a custom path
    python scripts/make_igtopt_template.py -o /tmp/my_template.xlsx

    # Print the list of sheets that would be generated (no file written)
...
# ---------------------------------------------------------------------------
# Field metadata: name → (type_hint, required, description, example)
# These descriptions are derived from the C++ struct comments and the
# gtopt documentation.  The "example" column is pre-filled in the sheet.
# ---------------------------------------------------------------------------
...
FIELD_META: dict[str, list[tuple[str, str, bool, str, Any]]] = {
    ...
    "converter_array": [
        ...
        ("active", _J_INT, False, "1 = active, 0 = inactive (default: 1)", None),
        ("battery", _J_ID, True, "Battery uid or name linked by this converter", "bat1"),
        (
            "generator",
            _J_ID,
            True,
...
_INTRO_LINES = [
    ...
    ("", ""),
    ("Generated by make_igtopt_template.py — re-run to refresh after C++ changes.", "FOOTER"),
]
...
_EXAMPLES: dict[str, list[dict[str, Any]]] = {
    ...
    "stage_array": [
        {"uid": 1, "name": "s1", "first_block": 0, "count_block": 8760}
    ],
    "scenario_array": [
...
def parse_json_header_fields(header_path: Path) -> list[str]:
    ...
    # Find all json_member_list blocks
    # We look for json_member_list< ... > and extract "field_name" patterns
    # Strategy: find all quoted names that follow json_*<
    fields = _FIELD_NAME_RE.findall(text)
    # Remove duplicates while preserving order; remove internal schema names
    seen: set[str] = set()
    result = []
    for f in fields:
        if f not in seen:
...
def parse_system_arrays(json_system_hpp: Path) -> list[str]:
    ...
    # Match json_array_null<"xxx_array", ...>
    return re.findall(r'json_array_null<"([^"]+)"', text)


def parse_simulation_arrays(json_simulation_hpp: Path) -> list[str]:
...
def _build_workbook(output_path: Path, header_dir: Path) -> None:  # noqa: PLR0912,PLR0915
    ...
    def _add_data_sheet(
        ...
        # Row 2: help text (type/required/description) — in italics with fill
        ...
        leg_label = ws_data.cell(
            row=1, column=legend_col, value="Legend"
        )
    ...
    # Simulation sheets
    # ------------------------------------------------------------------
    for sheet in _SIMULATION_SHEETS:
    ...
    for col_idx, header in enumerate(["scenario", "stage", "block", "d1", "d2"], start=1):
...
def _list_sheets(header_dir: Path) -> None:
...
def main(argv: list[str] | None = None) -> int:
    """Entry point for make_igtopt_template."""
    ...
    ap.add_argument(
        ...
        metavar="DIR",
        help=(
            "Path to include/gtopt (auto-detected from the repo root "
            "if omitted)"
        ),
    )
    ap.add_argument(
        ...
        help=(
            "Output .xlsx path "
            "(default: docs/templates/gtopt_template.xlsx relative to repo root)"
        ),
    )
    ap.add_argument(
        "--list-sheets",
        action="store_true",
        help="Print the list of sheets parsed from C++ headers and exit",
    )
    ...
    if not header_dir.is_dir():
        print(
            f"Error: header directory '{header_dir}' not found.",
            file=sys.stderr,
        )
        return 1
