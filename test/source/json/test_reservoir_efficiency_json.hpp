...
TEST_CASE("EfficiencySegment daw json round-trip")
{
  ...
  const EfficiencySegment seg =
      daw::json::from_json<EfficiencySegment>(json_data);
...
TEST_CASE("EfficiencySegment daw json default values")
{
  std::string_view json_data = R"({
    "volume": 0.0,
    "slope": 0.0,
...
TEST_CASE("ReservoirEfficiency daw json test 1")
{
  std::string_view json_data = R"({
    "uid": 1,
    "name": "eff_colbun",
...
TEST_CASE("ReservoirEfficiency daw json test - multiple segments")
{
  std::string_view json_data = R"({
    "uid": 2,
    "name": "eff_eltoro",
  ...
  REQUIRE(re.mean_efficiency == doctest::Approx(4.8));
...
TEST_CASE("ReservoirEfficiency daw json test - empty segments")
{
  ...
  REQUIRE(re.uid == 3);
  REQUIRE(re.mean_efficiency == doctest::Approx(2.0));
  REQUIRE(re.segments.empty());
}

TEST_CASE("ReservoirEfficiency daw json array test")
{
  std::string_view json_data = R"([
    {
      "uid": 1,
  ...
  const std::vector<ReservoirEfficiency> effs =
      daw::json::from_json_array<ReservoirEfficiency>(json_data);
...
TEST_CASE("ReservoirEfficiency to_json and from_json round-trip")
{
  const ReservoirEfficiency orig {
      .uid = 7,
      .name = "eff_roundtrip",
      .turbine = Uid {101},
      .reservoir = Uid {201},
      .mean_efficiency = 1.75,
      .segments =
          {
              {.volume = 0.0, .slope = 0.0005, .constant = 1.5},
              {.volume = 800.0, .slope = 0.0002, .constant = 1.75},
          },
      .sddp_efficiency_update_skip = 2,
  };

  const auto json = daw::json::to_json(orig);
  const ReservoirEfficiency roundtrip =
      daw::json::from_json<ReservoirEfficiency>(json);
  ...
  CHECK(roundtrip.mean_efficiency == doctest::Approx(1.75));
