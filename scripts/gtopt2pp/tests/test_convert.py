...
class TestConvert:
    ...
    def test_minimal_case_buses(self) -> None:
        net = convert(_MINIMAL_CASE, scenario=0, block=0)
    ...
    def test_minimal_case_loads_block1(self) -> None:
        net = convert(_MINIMAL_CASE, scenario=0, block=1)
        loads = net.load.sort_values("name")
...
class TestConvertAllBlocks:
    ...
    def test_returns_list(self) -> None:
        nets = convert_all_blocks(_MINIMAL_CASE, scenario=0)
        assert len(nets) == 2
...
class TestGeneratorProfileConversion:
    ...
    def test_profile_applied(self) -> None:
        ...
        net_b0 = convert(case, scenario=0, block=0)
        net_b1 = convert(case, scenario=0, block=1)
