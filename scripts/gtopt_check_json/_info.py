...
def format_info(planning: dict[str, Any]) -> str:
    ...
    lines.append(f"  Generators      : {len(sys.get('generator_array', []))}")
    lines.append(f"  Generator profs : {len(sys.get('generator_profile_array', []))}")
    lines.append(f"  Demands         : {len(sys.get('demand_array', []))}")
    lines.append(f"  Demand profs    : {len(sys.get('demand_profile_array', []))}")
    lines.append(f"  Lines           : {len(sys.get('line_array', []))}")
    lines.append(f"  Batteries       : {len(sys.get('battery_array', []))}")
    lines.append(f"  Converters      : {len(sys.get('converter_array', []))}")
    lines.append(f"  Reserve zones   : {len(sys.get('reserve_zone_array', []))}")
    lines.append(
        f"  Reserve provisions   : {len(sys.get('reserve_provision_array', []))}"
    )
    lines.append(f"  Junctions       : {len(sys.get('junction_array', []))}")
    lines.append(f"  Waterways       : {len(sys.get('waterway_array', []))}")
    lines.append(f"  Flows           : {len(sys.get('flow_array', []))}")
    lines.append(f"  Reservoirs      : {len(sys.get('reservoir_array', []))}")
    lines.append(f"  Filtrations     : {len(sys.get('filtration_array', []))}")
    lines.append(f"  Turbines        : {len(sys.get('turbine_array', []))}")
