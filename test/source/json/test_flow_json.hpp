...
TEST_CASE("Flow JSON basic parsing")
{
  ...
  CHECK(std::get<Uid>(flow.junction.value()) == 10);
...
TEST_CASE("Flow JSON round-trip serialization")
{
  ...
  const Flow roundtrip = daw::json::from_json<Flow>(
      json);  // NOLINT(performance-unnecessary-copy-initialization)
  ...
  CHECK(std::get<Uid>(roundtrip.junction.value()) == 3);
