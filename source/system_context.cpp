...
std::span<const std::pair<ColIndex, double>>
SystemContext::find_ampl_weighted_cols(std::string_view class_name,
                                       Uid element_uid,
                                       std::string_view attribute,
                                       ScenarioUid scenario_uid,
                                       StageUid stage_uid,
                                       BlockUid block_uid) const
{
  return simulation().find_ampl_weighted_cols(system().scene().index(),
                                              system().phase().index(),
                                              class_name,
                                              element_uid,
                                              attribute,
