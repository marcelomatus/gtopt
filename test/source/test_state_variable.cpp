...
TEST_CASE("StateVariable::Key functionality") {
    ...
    SUBCASE("Key comparison") {
        const auto key1 = StateVariable::key(
            "ClassA", 1, "col1", PhaseIndex{1}, StageUid{1});
        const auto key2 = StateVariable::key(
            "ClassA", 1, "col1", PhaseIndex{1}, StageUid{1});
        const auto key3 = StateVariable::key(
            "ClassB", 1, "col1", PhaseIndex{1}, StageUid{1});
...
TEST_CASE("StateVariable core functionality") {
    ...
    SUBCASE("Dependent variables") {
        const auto& dep1 = var.add_dependent_variable(
            {.scene_index = SceneIndex{4}, .phase_index = PhaseIndex{5}}, 10);
        const auto& dep2 = var.add_dependent_variable(
...
TEST_CASE("StateVariable dependent variable templates") {
    ...
    SUBCASE("Template add_dependent_variable") {
        struct TestStageLP {
            PhaseIndex phase_index() const { return PhaseIndex{5}; }
        };
        struct TestScenarioLP {
            SceneIndex scene_index() const { return SceneIndex{4}; }
        };
