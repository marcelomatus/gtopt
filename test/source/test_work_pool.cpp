...
// ─── WorkPoolConfig load_factor knob ─────────────────────────────────────────

TEST_CASE("WorkPoolConfig load_factor default is disabled (0.0)")  // NOLINT
{
  // The base WorkPoolConfig keeps the load gate disabled by default
  // because small pools running under high system-wide load (e.g.
  // unit tests under `ctest -j20`) would otherwise see the gate fire
  // on background load that has nothing to do with this pool's own
  // work.  The SDDP factory `make_sddp_work_pool()` opts in to 1.25.
  const WorkPoolConfig cfg;
  CHECK(cfg.load_factor == doctest::Approx(0.0));
}

TEST_CASE("WorkPoolConfig load_factor is settable via constructor")  // NOLINT
{
  const WorkPoolConfig cfg {
      2,  // max_threads
      95.0,  // max_cpu_threshold
  ...
  CHECK(cfg.load_factor == doctest::Approx(1.5));
}

TEST_CASE("AdaptiveWorkPool: load gate disabled (0.0) admits work")  // NOLINT
{
  // Default config (load_factor=0) must admit work even when the
  // host's loadavg is high — the gate is opt-in, not opt-out.
  AdaptiveWorkPool pool(WorkPoolConfig {
      2,  // max_threads
      95.0,  // max_cpu_threshold
      4096.0,  // min_free_memory_mb
      ...
      0.0,  // max_process_rss_mb
      std::chrono::milliseconds(10),  // scheduler_interval
      "LoadDisabledPool",  // name
      false,  // enable_periodic_stats
      0.0,  // max_process_swap_mb
      0.0,  // max_swap_io_per_sec
      0.0,  // load_factor (disabled)
  });
  pool.start();

  auto f = pool.submit([] { return 99; });
  REQUIRE(f.has_value());
  CHECK(f.value().get() == 99);

  // Statistics surface loadavg + the new throttle counter even when
  // the gate is disabled — both are observability fields.
  std::this_thread::sleep_for(std::chrono::milliseconds(50));
  const auto stats = pool.get_statistics();
  CHECK(stats.loadavg_1min >= 0.0);
  CHECK(stats.throttled_load == 0);
}
