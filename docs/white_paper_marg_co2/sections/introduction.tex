...
Section~\ref{sec:related} reviews the international state of the
art on marginal-emission accounting and identifies the methodological
choice points specific to a public-data reconstruction.
Section~\ref{sec:methodology} develops the model and the
nine-step pipeline.  Section~\ref{sec:cases} reports on a
pedagogical IEEE 9-bus example, the production application to the
\num{29}-bus CEN system on 2026-04-22, and a sensitivity analysis.
Section~\ref{sec:results} consolidates the validation numbers
against $\lambda^{\mathrm{CEN}}$ and against the PLEXOS oracle.
Section~\ref{sec:discussion} surveys the methodology's structural
limitations.  Section~\ref{sec:conclusions} closes with a
forward-looking integration into the \texttt{gtopt} stochastic
expansion-planning solver.
