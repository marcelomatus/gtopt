...
4. [Domain Specifications](#4-domain-specifications)
5. [Examples](#5-examples)
6. [External Constraint Files](#6-external-constraint-files)
7. [Formal Grammar (BNF)](#7-formal-grammar-bnf)
8. [Comparison with AMPL and Mathematica](#8-comparison-with-ampl-and-mathematica)
...
Elements can be referenced by **name** or by **UID**:

```
generator("TORO")       -- matches generator with name "TORO"
generator("uid:23")     -- matches generator with uid = 23
demand("D1")            -- matches demand with name "D1"
line("L1_2")            -- matches line with name "L1_2"
battery("BESS1")        -- matches battery with name "BESS1"
```
...
element_ref    := element_type '(' STRING ')' '.' IDENT
...
## 8. Comparison with AMPL and Mathematica
...
# AMPL equivalent:
subject to gen_pair_limit {s in STAGES, b in BLOCKS: s in {4,5,6} and b >= 1 and b <= 30}:
  generation["G1",s,b] + generation["G2",s,b] <= 300;
```
...
| File format | `.mod` text file | JSON field or file |
| Scope | Full modeling language | LP constraints only |

### Mathematica LinearProgramming equivalent

```mathematica
(* gtopt: generator("G1").generation + generator("G2").generation <= 300 *)

(* Mathematica: *)
LinearProgramming[c, {{1, 1}}, {{300, -1}}]
(* where -1 means <= *)
```

The gtopt syntax is intentionally simpler than AMPL — it provides
element-level referencing without requiring explicit set definitions.
...
3. **Use UIDs for stability**: `generator("uid:5")` is stable across name
   changes; `generator("TORO")` breaks if the generator is renamed.

4. **Set `active: false` for debugging**: disable a constraint without
   removing it from the file.

5. **Use external files for large constraint sets**: when you have more than
   ~10 constraints, move them to a separate file referenced by
   `user_constraint_file`.

6. **Validate with `use_single_bus: true` first**: check that your
   constraints are feasible in a simple model before adding network
   constraints.

7. **Check LP feasibility**: if adding user constraints makes the problem
