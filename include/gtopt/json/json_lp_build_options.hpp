...
{
  [[nodiscard]] LpBuildOptions operator()(
      OptInt names_level_int, OptReal lp_coeff_ratio_threshold) const
  {
    LpBuildOptions opts;
    if (names_level_int) {
...
struct json_data_contract<LpBuildOptions>
{
  ...
  static auto to_json_data(LpBuildOptions const& opt)
  {
    const OptInt names_int = opt.names_level
        ...
        : OptInt {};
    return std::make_tuple(names_int, opt.lp_coeff_ratio_threshold);
  }
};
