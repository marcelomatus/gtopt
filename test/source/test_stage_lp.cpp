...
TEST_SUITE("StageLP") {
    TEST_CASE("Construction and basic properties") {
        std::vector<Block> blocks = {
            Block{.duration = 12.0, .uid = 1},
            Block{.duration = 24.0, .uid = 2},
            Block{.duration = 24.0, .uid = 3},
            Block{.duration = 12.0, .uid = 4}
        };
        ...
        SUBCASE("Duration calculations") {
            ...
            CHECK(stage_lp.timeinit() == doctest::Approx(12.0)); // first block's duration
        }
        ...
        SUBCASE("Discount factor calculations") {
            ...
            CHECK(stage_lp.discount_factor() == doctest::Approx(0.9 * 0.999933).epsilon(1e-6));
        }
    ...
    TEST_CASE("Edge cases") {
        ...
        SUBCASE("No stage discount factor") {
            ...
            CHECK(stage_lp.discount_factor() == doctest::Approx(0.999933).epsilon(1e-6));
        }
    }
