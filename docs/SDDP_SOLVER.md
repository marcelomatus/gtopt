...
```bash
gtopt my_case.json --trace-log sddp_trace.log
```
...
The SDDP solver uses `std::mutex` to protect shared data structures
(stored cuts) during parallel scene processing.  Each scene has its own
per-phase LP subproblems, so forward/backward passes for different scenes
can proceed in parallel without LP-level locking.
