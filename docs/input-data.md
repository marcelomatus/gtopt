...
| `low_memory_mode`      | string  | `"compress"` (SDDP/cascade) / `"off"` (monolithic) | `off` or `compress` (`snapshot` and `rebuild` are back-compat aliases for `compress`; see SDDP method docs) |
