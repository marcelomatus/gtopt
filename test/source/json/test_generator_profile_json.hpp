...
TEST_CASE("GeneratorProfile daw json test - with cost and active")
{
  ...
  CHECK(std::get<IntBool>(gp.active.value()) == True);  // NOLINT(bugprone-unchecked-optional-access)
  CHECK(std::get<Name>(gp.generator) == "GEN_REF");
  REQUIRE(gp.scost.has_value());
  CHECK(std::get<double>(gp.scost.value())  // NOLINT(bugprone-unchecked-optional-access)
