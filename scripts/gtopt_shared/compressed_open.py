...
Shared by every converter (plp2gtopt, sddp2gtopt, psse2gtopt) so they
all read .xz/.gz/.zst/.lz4 inputs transparently, with variant + split
probing.
...
def open_binary(path: Path) -> IO[bytes]:
    """Resolve + open a (possibly compressed) file for **binary** reading.

    For Excel ``.xls``/``.xlsx`` stored as ``.xls.xz``: resolves the
    compressed variant and returns a decompressed binary stream the
    caller hands to pandas/openpyxl.
    """
    resolved = resolve_compressed_path(Path(path))
    opener = _OPENER.get(resolved.suffix.lower())
    if opener is not None:
        return opener(resolved)
    return open(resolved, "rb")  # noqa: SIM115  # pylint: disable=consider-using-with


def read_text(path: Path, encoding: str = "utf-8", errors: str = "strict") -> str:
    """Resolve + read a (possibly compressed) text file fully into a str."""
    resolved = resolve_compressed_path(Path(path))
    with compressed_open(resolved, encoding=encoding, errors=errors) as fh:
        return fh.read()
