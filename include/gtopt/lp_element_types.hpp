 ...
 *
 * `UserConstraintLP` is the last OPERATIONAL type so that user constraints are
 * added to the LP after all other elements (user constraints reference
 * columns created by the earlier elements).  `UserModelLP` (generic AMPL
 * capture) follows it — it reuses the same resolver/registry via internal
 * delegates, so it must run after the columns it may reference exist.
 * `FutureCostLP` is last, planning-only (no per-(scenario, stage) `add_to_lp`),
 * so it does not affect the operational build order; it is placed after both
 * ReservoirLP and UserConstraintLP so its α cut can reference their terminal
 * state columns.
 */
// NOTE: ConverterLP runs AFTER CommitmentLP / SimpleCommitmentLP so
// that the synthesized battery commitment (generated by
// `System::expand_batteries()` for `<bat>_gen`) has already stamped
// its `status` (u_commit) cols onto the LP.  ConverterLP then looks
// up that single u_commit and uses it to gate the charge-side
// demand bounds and any minimum-throughput row — the "one true
// source for u_commit" principle.
